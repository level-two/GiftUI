...
def emit_payload(type_name: str, payload: bytes, records: Sequence[dict]) -> str:
    lines = [
        "// Generated by scripts/text-resources/generate-reference-resources.py. Do not edit.",
        "import GiftUITextResources",
        "",
    ...
    for record in records:
        start = record["offset"]
        record_bytes = payload[start:start + record["byteCount"]]
        if not record_bytes:
            lines.append(f"    static let glyph{record['glyph']}: () = ()")
            continue
        lines.append(f"    static let glyph{record['glyph']} = (")
        for index in range(0, len(record_bytes), 16):
            values = ", ".join(
                f"0x{value:02x} as UInt8"
                for value in record_bytes[index:index + 16]
            )
    ...
    for record in records:
        lines.extend([
            f"        case {record['glyph']}:",
            f"            return try withUnsafeBytes(of: glyph{record['glyph']}) {{ bytes in",
...
def generate(source: Path, license_path: Path, output: Path, pins: dict) -> None:
    ...
    outputs = {
        ...
        "ReferenceBitmapPayload.generated.swift": emit_payload(
            "_GiftUIReferenceGeneratedBitmapPayload",
            bytes(bitmap_payload),
            bitmap_records,
        ),
        "ReferenceOutlinePayload.generated.swift": emit_payload(
            "_GiftUIReferenceGeneratedOutlinePayload",
            bytes(outline_payload),
            outline_records,
        ),
    }
