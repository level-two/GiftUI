...
failures=$((failures + 1))
printf 'resource-instrumentation\tblocked\tT7.2 resource and timing fields are not implemented\n' >>"${prerequisites_path}"
...
printf 'SPEC-013 %s report finalized with T7.2 explicitly blocked; run ID: %s\n' "${profile}" "${run_id}" >&2
exit 1
