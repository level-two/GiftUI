...
fail() {
    printf 'SPEC-008 value profile check failed: %s\n' "$*" >&2
    exit 1
}
...
if [[ -z "${output_root}" ]]; then
    temporary_root="$(mktemp -d "${TMPDIR:-/tmp}/giftui-spec008-values.XXXXXX")"
...
run_command "${compiler}" "${common_module_flags[@]}" -I "${module_dir}" \
    -module-name GiftUIRenderLowering \
    "${PROJECT_ROOT}/Sources/GiftUIRenderLowering/RenderProductionValues.swift" \
    -emit-module-path "${module_dir}/GiftUIRenderLowering.swiftmodule" >/dev/null

layout_ir="${output_root}/render-value-layouts.ll"
layout_report="${output_root}/render-value-layouts.tsv"
run_command "${compiler}" "${flags[@]}" -parse-as-library -package-name GiftUI \
    -I "${module_dir}" -emit-ir -module-name GiftUIRenderValueLayoutProbe \
    "${FIXTURE_ROOT}/Instrumentation/RenderValueLayoutProbe.swift" \
