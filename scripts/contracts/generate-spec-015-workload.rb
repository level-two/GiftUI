...
def parse_descriptor
  values = {}
  INPUT.read.lines.each_with_index do |line, index|
    next if line.start_with?("#") || line.strip.empty?
    key, value, extra = line.chomp.split("\t", -1)
    fail_generation("invalid descriptor row #{index + 1}") if key.nil? || value.nil? || extra
    fail_generation("duplicate descriptor field #{key}") if values.key?(key)
    values[key] = value
  end
  values
end
...
descriptor_hash = Digest::SHA256.hexdigest(INPUT.read)
