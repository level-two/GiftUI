...
fixture_root = root.join("Tests/ContractFixtures/SPEC005")
manifest_path = fixture_root.join("fixture-manifest.tsv")

def fail_check(message)
  warn "SPEC-005 fixture manifest check failed: #{message}"
  exit 1
end
...
required_paths = %w[
  ...
  generated-assets.tsv
  SemanticCorpus/README.md
  SemanticCorpus/cases.tsv
  ResourceHarness/README.md
  ResourceHarness/Baseline/README.md
  ResourceHarness/Candidate/README.md
]
required_paths.each do |relative|
  fail_check("required fixture path is missing: #{relative}") unless fixture_root.join(relative).file?
end
...
registered_directories = rows.map do |id, expectation, access, entry_path, patterns_path|
  ...
  if expectation == "pass"
    ...
    meaningful = patterns.each_line.any? { |entry| !entry.strip.empty? && !entry.lstrip.start_with?("#") }
...
fixture_directories = fixture_root.join("Fixtures").glob("{Positive,Negative}/*")
  .select(&:directory?).map { |path| path.realpath.to_s }
unless registered_directories.sort == fixture_directories.sort
  fail_check("registered fixture directories differ from the checked-in layout")
end

puts "SPEC-005 fixture manifest check passed: #{rows.length} fixtures."
