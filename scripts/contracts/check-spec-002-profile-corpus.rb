...
CORPUS = File.join(ROOT, "Tests/ContractFixtures/SPEC002/SemanticCorpus/cases.tsv")
PROBE = File.join(
  ROOT,
  "Tests/ContractFixtures/SPEC002/ProfileCorpusProbe/ProfileCorpusProbe.swift"
)
MAIN = File.join(ROOT, "Tests/ContractFixtures/SPEC002/ProfileCorpusProbe/main.swift")

def fail_check(message)
  warn "SPEC-002 profile corpus check failed: #{message}"
  exit 1
end
...
rows.each_with_index do |line, index|
  ...
  fail_check("invalid corpus id #{id.inspect}") unless id.match?(/\A[a-z0-9-]+\z/)
...
puts "SPEC-002 profile corpus check passed: #{ids.length} ordered cases, checksum #{checksum}."
