...
physical-input path. It provenance-stamps presentation-coupled input when the
corresponding physical presentation is eligible and otherwise drops it before
runtime admission. Core validates provenance again at admission and cancels
...
| B10 | Platform/input driver adapter -> semantic runtime admission | `GiftUI`-owned backend-neutral pointer/touch value, logical coordinates, phase, bounded source/sequence identity, eligible physical-presentation provenance for presentation-coupled events, and bounded ordering metadata | Drivers own sampling, calibration, interrupts, evdev, or mouse records; a sibling input/platform adapter owns normalization and feeds the RFC-004 admission contract only after B16 presentation eligibility permits it. Input does not enter through the render-oriented `GiftUIBackend` SPI. The runtime validates provenance and sequence identity, owns pointer sequencing, hit testing, disabled-state enforcement, and semantic routing; producers never invoke client actions directly. |
...
| B16 | Presentation integration -> sibling physical-input admission gate | Committed frame provenance, backend-local presentation eligibility/health, and the minimum bounded source/sequence state needed to stamp eligible presentation-coupled input or invalidate a dropped sequence and resynchronize its source | RFC-004 owns the fail-closed coherence and sequencing rule. Target integrations coordinate presentation and physical input without deferring events for later retargeting, exposing device/transport failure as GiftUI semantics, rolling back committed state, replaying work, hit testing, or invoking handlers. |
...
| B6 | Every first-party MVP backend consumes the one-shot ordered operation stream synchronously during frame offer. Before accepting, it reserves bounded capacity and retains or transfers only backend-owned derived pixel, transfer, or device data. Refusal retains nothing and makes no irreversible presentation effect. Once irreversible output begins, responsibility has transferred and the outcome is accepted. The coordinator may retain only constant-space latest-revision presentation intent after retryable refusal, never the refused payload. | Frame offer and logical commit-or-abort are synchronous. Retryable refusal requests a separately paced rederivation under a finite target policy. Presentation may continue asynchronously below the handoff boundary after acceptance; its outcomes do not re-enter Core to alter frame disposition or call semantic code. |
...
| B16 | The target presentation/input integration owns bounded local eligibility and health state for the accepted frames it advances, plus the bounded source phase and sequence identity needed to invalidate a dropped sequence, discard its orphaned phases, and regain physical synchronization. No Core completion queue or deferred-input queue is required for that state. | Presentation completion may be asynchronous, but the integration locally sequences it with physical input, provenance-stamps events known to correspond to an eligible presentation, and otherwise drops them. A later down uses a distinct sequence identity so Core clears older capture; no later phase from the invalidated sequence is retargeted. |
...
| B6 | Complete accepted handoff commits the logical frame; preparation failure or side-effect-free handoff refusal aborts it. Beginning irreversible presentation output requires an accepted disposition. Retryable refusal retains only constant-space latest-revision presentation intent, triggers separately paced rederivation without effect or payload replay, and follows a finite terminal policy. Post-handoff presentation failure never changes that disposition or rolls back semantic state. Backend-owned downstream slots and presentation data are bounded. | Integration SPI | Recording/checking backend contract suite, refusal-before-effect checks, pending-intent coalescing and retry-exhaustion fixtures, effect/action non-replay checks, post-handoff fault injection after irreversible output, and derived-presentation-data lifetime instrumentation. |
...
| B8 | Invalid geometry/format, insufficient reserved capacity, or synchronous submission refusal prevents handoff acceptance only before irreversible presentation output. Once output begins, disconnection, partial presentation, and downstream failure are bounded local health/recovery conditions after responsibility transfer. | Integration SPI | Fake display-target fault injection plus Raspberry Pi and nRF52840 connected-display evidence proving refusal has no irreversible effect, output begins only after responsibility transfer, and post-handoff behavior preserves local presentation/input coherence. |
...
| B10 | Malformed samples, coordinate overflow, stale provenance, queue exhaustion, dropped sequence phases, and unsupported event forms have deterministic drop/cancel/failure/diagnostic behavior. Event queues, batch size, source count, provenance, and identifiers are bounded. | Integration SPI feeding framework SPI | Adapter fixtures for mouse/host, evdev, and embedded touch; ordering, calibration, every overflow boundary, stale-revision races, sequence resynchronization, disabled-hit, and revision-correlation tests independent of a pixel backend. |
| B11 | Missing/stale captured action identity, changed release hit, disabled action, orphaned phase, handler failure, state-slot exhaustion, and reentrant input have explicit behavior; action/state slots, per-source pointer state, and queued invalidations are bounded. | Client API plus framework SPI | Cross-profile fixtures for revision changes during a press, identity change/removal, hit movement, state preservation/removal, disabled controls, ordered actions, reentrancy, coalescing, and deterministic exhaustion. |
...
| B16 | Presentation-coupled input is stamped and forwarded only when its physical presentation is eligible; otherwise it is dropped and the affected pointer sequence is cancelled. No event is deferred for later retargeting. Local eligibility and source-resynchronization state are bounded with deterministic exhaustion behavior. | Integration SPI below framework admission | Presentation/input fixtures covering delayed completion, failure, recovery, rapid accepted frames, stale physical events, gating saturation, dropped down/move/up phases, orphan suppression, and targets that cannot claim interactive coherence. |
...
1. **Operation ownership and the logical commit boundary are resolved in the
   ...
   before irreversible output; retryable refusal retains constant-space intent
   for the latest published revision and requests finite, separately paced
   rederivation without replaying client effects or the refused payload. Later
   presentation and device health remain in the target integration, which
   coordinates them with physical input. Presentation-coupled events carry
   eligible physical-presentation provenance, are validated again at runtime
   admission, and fail closed by dropping and cancelling their pointer
   sequence rather than being deferred or retargeted. Replayable operation
...
10. Backend-neutral input normalization is a sibling integration seam feeding
    ...
    maps remain parallel to render payloads. Presentation-coupled events carry
    provenance, stale events are dropped without retargeting, and Core owns
    bounded pointer sequencing plus current-revision hit, identity, and
