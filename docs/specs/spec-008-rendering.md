...
semantic-style resolution, and deterministic lowering of resolved layout into
a normalized ordered stream of rectangle-fill and positioned-glyph operations.
...
- semantic-to-resolved-layout-to-render lowering;
...
- ADR-005, ADR-006, ADR-021, ADR-022, and ADR-023 are accepted.
...
`GiftUIRenderCore` owns style resolution, render lowering, normalized payloads,
limits/results, and the recording sink. It MUST depend on `GiftUI`,
`GiftUISemanticCore`, and `GiftUITextResources`. It consumes SPEC-007 output
through a coordinator-owned adapter and MUST NOT import `GiftUILayout`; the two
physical modules remain siblings as required by RFC-003.

`GiftUIRenderCore` MUST NOT import runtime-profile implementations, backends,
raster providers, capabilities, concrete resources, platform, driver, OS/RTOS,
HAL, or hardware modules. It MUST NOT import `GiftUIFailureCore`; the first
owner adapter that knows both contracts performs Error Handling mappings.

Backends and raster providers consume the normalized SPI but MUST NOT import
`GiftUI`, semantic core, or layout to reinterpret content. An implementation
MUST NOT create a parallel `Color`, `FontInstanceID`, or `GlyphID` at a lower
boundary.
...
The render entry point synchronously borrows a complete semantic result, the
corresponding successful SPEC-007 result, the selected validated text package,
...
The root effective foreground is a required host input. A
...
Before `begin`, the producer MUST complete validation and prove that declared
limits and sink-reported capacity admit the whole candidate stream. `begin` is
called once, operations are delivered in order, and `finish` is called once.
...
Workspace and sink are exclusively borrowed for one attempt. Reentry fails
...
The producer does not resolve capabilities. A host must have validated the
required raster-presentation family before the runtime is admitted, but an
...
A conforming backend-side consumer MUST support every normalized operation in
this contract, consume each once during the later synchronous frame offer, and
...
The owner adapter maps errors to SPEC-003 facts:
...
- Import tests reject semantic/layout authority in backends and reject
  `GiftUIRenderCore` imports of layout implementations or concrete integrations.
...
- [ ] Import-graph tests preserve the semantic/layout/render/backend boundary
  and the `GiftUITextResources` identity owner.
