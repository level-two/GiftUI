...
- [ ] `T6.8` — After the concrete nRF52840 TFT/input assembly is explicitly
      ...
      **In progress:** the concrete assembly is now the
      ...
      The portable fixed UTF-8 diagnostic value now compiles in the Embedded
      Swift target and lives inline in the fixed model location. Host and
      ...
      production loop.
      The target now compiles the exact portable deterministic generator and
      wraps it in a fixed-value source with one-transition polling, initial
      four-low emission, generation checks, stop/restart, and shutdown. A host
      fixture compares 2,400 scheduled transitions and delays against the
      portable generator. Firmware startup checks its first event and stale
      ...
      The target-safe layout workspace now owns the exact disjoint scope and
      render slices for one attempt, with unique scope appends, measurement
      replacement, placement, a bounded 13-entry depth stack, reset, and
      ...
      open.
      The target text measure helper now uses the published UTF-8 view and
      canonical font projection to stage glyphs, lines, and compact scope
      measurements with the common CR/LF and wrap behavior. Its differential
      fixture compares both complete profile regions against the common
      measure engine for every diagnostic text scope, including 96 LF bytes.
      The firmware startup probe exercises the helper on the title. A former
      primitive decoder bound used scalar capacity for UTF-8 byte offsets;
      this is corrected to the generated byte-pool capacity. Container and
